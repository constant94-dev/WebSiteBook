...
//echo $search;
    ...
    // bct_domestic_crawl 테이블에 검색어 관련된 전체 행 수
    //$DcrawlSQL = "SELECT count(*) AS cnt_Dcrawl FROM bct_domestic_crawl WHERE title LIKE '%$search%' AND content LIKE '%$search%'";
    // bct_foreign_crawl 테이블에 검색어 관련된 전체 행 수
    //$FcrawlSQL = "SELECT count(*) AS cnt_Fcrawl FROM bct_foreign_crawl WHERE title LIKE '%$search%' AND content LIKE '%$search%'";
    // bct_webtoon_crawl 테이블에 검색어 관련된 전체 행 수
    //$WcrawlSQL = "SELECT count(*) AS cnt_Wcrawl FROM bct_webtoon_crawl WHERE webtoonTitle LIKE '%$search%'";
    // 연결된 데이터베이스에 query를 보내는 명령어
	$result = $db->query($searchSQL);
    // 객체 지향 스타일 결과 집합에서 가져온 행을 나타내는 문자열의 연관 배열을 반환합니다   
    $row = $result->fetch_assoc();
    // FAQ 검색된 수
    $allBoared = $row['cnt_search'];
    
    //$Dcrawlresult = $db->query($DcrawlSQL);
    //$Dcrawlrow = $Dcrawlresult->fetch_assoc();
    // 국내도서 검색된 수
    //$allDcrawl = $row['cnt_Dcrawl'];
    
    //$Fcrawlresult = $db->query($FcrawlSQL);
    //$Fcrawlrow = $Fcrawlresult->fetch_assoc();
    // 외국도서 검색된 수
    //$allFcrawl = $row['cnt_Fcrawl'];

    //$Wcrawlresult = $db->query($WcrawlSQL);
    //$Wcrawlrow = $Wcrawlresult->fetch_assoc();
    // 웹툰 검색된 수
    //$allWcrawl = $row['cnt_Wcrawl'];
	...
	$allPage = ceil($allBoared / $onePage); //전체 페이지의 수
